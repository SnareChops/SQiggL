 ...
 */
export default class LengthLessThan extends Condition implements ICondition {
    /** 
     ...
     */
	public static regex: RegExp = new RegExp(`(\\w+)\\s+((?:${LengthLessThan.mods(LengthLessThan)}|\\s*))len<((?:${LengthLessThan.mods(LengthLessThan)}|\\s*))\\s+(\\d+)`, 'i');
	...
	constructor(public variable: string, public variables: IVariables, public comparative: string, mod1?: string, mod2?: string){
        super();
        this.modifiers = super.extractModifiers(LengthLessThan, mod1, mod2);
    }
    /**
    ...
    public static extract(statement: string, variables: IVariables){
        let match = statement.match(LengthLessThan.regex);
        if(match && match.length > 0) return new LengthLessThan(match[1], variables, match[4], match[2], match[3]);
        return null;
    }
